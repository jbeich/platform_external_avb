...
use super::{util::extract_slice, DescriptorError, DescriptorResult};
...
impl<'a> HashDescriptor<'a> {
    ...
    pub(super) fn new(contents: &'a [u8]) -> DescriptorResult<Self> {
        ...
        let (_, remainder) = extract_slice(contents, size_of::<AvbHashDescriptor>())?;
        let (partition_name, remainder) = extract_slice(remainder, header.partition_name_len)?;
        let (salt, remainder) = extract_slice(remainder, header.salt_len)?;
        let (digest, _) = extract_slice(remainder, header.digest_len)?;

        // Extract the hash algorithm from the original header since the temporary `header`
        // doesn't live past this function.
        //
        // SAFETY: we have verified that `contents` is a valid `AvbHashDescriptor`, and we
        // hold onto a borrow of this data so it will not change while we exist.
        let raw_header = unsafe { (contents.as_ptr() as *const AvbHashDescriptor).as_ref() };
        let raw_header = raw_header.unwrap();
        // The hash algorithm is a nul-terminated UTF-8 string.
...
mod tests {
    ...
    fn new_hash_descriptor_success() {
        ...
        assert_eq!(descriptor.unwrap().raw_data, TEST_HASH_DESCRIPTOR);
    }
