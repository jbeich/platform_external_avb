...
pub enum DescriptorError {
    ...
    InvalidHeader,
    /// A given value was too large and triggered math overflow.
    Overflow,
}
...
// Bindgen creates these values as u32 but in the headers they are u64. There doesn't seem to be
// a way to tell bindgen what type to use so convert them here.
const DESCRIPTOR_TAG_PROPERTY: u64 = AvbDescriptorTag::AVB_DESCRIPTOR_TAG_PROPERTY as u64;
const DESCRIPTOR_TAG_HASHTREE: u64 = AvbDescriptorTag::AVB_DESCRIPTOR_TAG_HASHTREE as u64;
const DESCRIPTOR_TAG_HASH: u64 = AvbDescriptorTag::AVB_DESCRIPTOR_TAG_HASH as u64;
const DESCRIPTOR_TAG_KERNEL_CMDLINE: u64 =
    AvbDescriptorTag::AVB_DESCRIPTOR_TAG_KERNEL_CMDLINE as u64;
const DESCRIPTOR_TAG_CHAIN_PARTITION: u64 =
    AvbDescriptorTag::AVB_DESCRIPTOR_TAG_CHAIN_PARTITION as u64;
...
impl<'a> Descriptor<'a> {
    ...
    unsafe fn new(raw_descriptor: *const AvbDescriptor) -> DescriptorResult<Self> {
        ...
        let num_bytes_following = descriptor
            .num_bytes_following
            .try_into()
            .map_err(|_| DescriptorError::Overflow)?;
        ...
        let contents = unsafe { from_raw_parts(raw_descriptor as *const u8, total_size) };
...
pub(crate) unsafe fn get_descriptors(vbmeta: &VbmetaData) -> DescriptorResult<Vec<Descriptor>> {
    let mut result = Ok(Vec::<Descriptor>::default());
