...
impl<'a> SlotVerifyData<'a> {
    ...
    /// * the lifetime `'a` must be the same lifetime as the provided `Ops<'a>`
...
pub fn slot_verify<'a>(
    ...
    let data = match out_data.is_null() {
        ...
        // * `'a` is the lifetime of the `Ops` preloaded partition data
        false => Some(unsafe { SlotVerifyData::<'a>::new(out_data)? }),
    };
