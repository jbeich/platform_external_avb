...
pub trait Ops {
    ...
    /// * `None` if the requested partition has not been preloaded; verification will attempt to
    ///   load the partition via `read_from_partition()`.
    /// * `IoError` if an error occurred; verification will exit immediately.
    fn get_preloaded_partition(&mut self, partition: &str) -> Result<Option<&[u8]>, IoError> {
        Ok(None)
    }
}
...
fn check_nonnull<T>(ptr: *const T) -> Result<*const T, IoError> {
    match ptr.is_null() {
        true => Err(IoError::Io),
        false => Ok(ptr),
    }
...
/// Converts a non-NULL `ptr` to `Ok(ptr)`, NULL to `Err(IoError::Io)`.
fn check_nonnull_mut<T>(ptr: *mut T) -> Result<*mut T, IoError> {
    match ptr.is_null() {
        true => Err(IoError::Io),
        false => Ok(ptr),
    }
...
unsafe fn try_read_from_partition(
    ...
    unsafe { ptr::write(check_nonnull_mut(out_num_read)?, 0) };
    // SAFETY:
    ...
    let partition = unsafe { CStr::from_ptr(check_nonnull(partition)?) };
    // SAFETY:
    // * we check for `NULL`.
    ...
    let buffer =
        unsafe { slice::from_raw_parts_mut(check_nonnull_mut(buffer)? as *mut u8, num_bytes) };

    let bytes_read = ops.read_from_partition(partition.to_str()?, offset, buffer)?;
    // SAFETY:
    // * we check for `NULL`.
    // * libavb gives us a properly-allocated `out_num_read`.
    unsafe { ptr::write(check_nonnull_mut(out_num_read)?, bytes_read) };
    Ok(())
}
...
unsafe fn try_get_preloaded_partition(
    ...
    unsafe {
        ptr::write(check_nonnull_mut(out_pointer)?, ptr::null_mut());
        ptr::write(check_nonnull_mut(out_num_bytes_preloaded)?, 0);
    }
    // SAFETY:
    // * we only use `ops` objects created via `create_avb_ops()` as required.
    // * `ops` is not held past the scope of this callback.
    ...
    if let Some(contents) = ops.get_preloaded_partition(partition.to_str()?)? {
        // SAFETY:
        // * we check for `NULL`.
        // * libavb gives us properly-aligned and sized `out` vars.
        unsafe {
            ptr::write(
                check_nonnull_mut(out_pointer)?,
            ...
            ptr::write(
                check_nonnull_mut(out_num_bytes_preloaded)?,
...
mod tests {
    ...
    impl Ops for TestOps {
        ...
        fn get_preloaded_partition(&mut self, partition: &str) -> Result<Option<&[u8]>, IoError> {
            Ok(self.preloaded.get(partition).map(|vec| &vec[..]))
        }
    }
