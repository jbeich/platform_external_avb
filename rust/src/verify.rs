...
use crate::{
    ...
    util::{buffer_to_slice, buffer_to_slice_mut, cstr_to_str, write_checked},
    IoError,
};
...
unsafe fn try_read_from_partition(
    ...
    // * we only access the contents via the returned `buffer`.
    // * contents are valid for the entire `lifetime`.
    let buffer = unsafe { buffer_to_slice_mut(buffer as *mut _, num_bytes, &lifetime) }?;
...
unsafe fn try_get_preloaded_partition(
    ...
    // * contents are valid for the entire `lifetime`.
    let partition = unsafe { cstr_to_str(partition, &lifetime) }?;
    let (ptr, size) = match ops.get_preloaded_partition(partition) {
        Some(contents) => {
    ...
    // unknown partition name libavb will fall back to `read_from_partition()` and handle the error
    // there.
    Ok(())
}
...
/// # Safety
/// * `ops` must have been created via `create_avb_ops()`.
/// * `public_key_*` args must adhere to the requirements of `buffer_to_slice()`.
/// * `out_is_trusted` must adhere to the requirements of `write_checked()`.
unsafe fn try_validate_vbmeta_public_key(
    ...
    // * contents are valid for the entire `lifetime`.
    let public_key = unsafe { buffer_to_slice(public_key_data, public_key_length, &lifetime) }?;
    // SAFETY:
    // * libavb gives us a properly-allocated `public_key_metadata` with size
    //   `public_key_metadata_length`.
    // * contents are valid for the entire `lifetime`.
    let metadata =
        unsafe { buffer_to_slice(public_key_metadata, public_key_metadata_length, &lifetime) }.ok();
    let (trusted, result) = match ops.validate_vbmeta_public_key(public_key, metadata) {
        Ok(trusted) => (trusted, Ok(())),
        // libavb should ignore this value on error, but for added safety we fail-closed
        // by saying the key is not trusted.
        Err(e) => (false, Err(e)),
    };
    // SAFETY:
    // * libavb gives us a properly-aligned and sized `out_is_trusted`.
    unsafe { write_checked(out_is_trusted, trusted) }?;
    result
}
