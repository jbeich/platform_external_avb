...
use crate::{
    ...
    util::{check_nonnull_mut, cstr_to_str},
    IoError,
};
...
/// * `buffer` must adhere to the requirements of `from_raw_parts_mut()`.
...
unsafe fn try_read_from_partition(
    ...
    // * the returned `&str` is not held past the scope of this callback.
    let partition = unsafe { cstr_to_str(partition) }?;
    // SAFETY:
    // * we check for `NULL`.
...
unsafe fn try_get_preloaded_partition(
    ...
    unsafe {
        ptr::write(check_nonnull_mut(out_pointer)?, ptr);
        ptr::write(check_nonnull_mut(out_num_bytes_preloaded)?, size);
    }
    Ok(())
}
