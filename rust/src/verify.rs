...
pub trait Ops {
    ...
    /// hashtree verification mode, and can be left unimplemented otherwise.
    ///
    /// # Arguments
    ...
    /// # Arguments
    /// * `name`: persistent value name.
    /// * `value`: bytes to write as the new value, or None to erase the persistent value.
    ///
    /// # Returns
    /// * Unit on success.
    ...
    /// * Any other `IoError` on failure.
    fn write_persistent_value(&mut self, name: &CStr, value: Option<&[u8]>) -> Result<()> {
        Err(IoError::NotImplemented)
    }
}
...
unsafe fn try_read_persistent_value(
    ...
    if let Err(IoError::InsufficientSpace(needed)) = result {
        // Extract the space needed from the error and pass it back to libavb.
        // SAFETY:
        // * we've checked that the pointer is non-NULL.
        // * libavb gives us a properly-allocated `out_num_bytes_read`.
        unsafe { ptr::write(out_num_bytes_read, needed) };
    };
    let size = result?;
...
unsafe fn try_write_persistent_value(
    ...
    // * the returned `&CStr` is not held past the scope of this callback.
    let name = unsafe { CStr::from_ptr(name) };
    let value = check_nonnull(value).ok().map(
        // SAFETY:
        // * we've checked that the pointer is non-NULL.
        // * libavb gives us a properly-allocated `value` with size `value_size`.
        // * we only access the contents via the returned slice.
        // * the returned slice is not held past the scope of this callback.
        |_| unsafe { slice::from_raw_parts(value, value_size) },
    );
...
mod tests {
    ...
    impl Ops for TestOps {
        ...
        fn write_persistent_value(&mut self, name: &CStr, value: Option<&[u8]>) -> Result<()> {
            ...
            match value {
                None => self.persistent_values.remove(name),
                Some(value) => self
                    .persistent_values
    ...
    fn test_read_persistent_value() {
        ...
        ops.persistent_values
            .insert("foo".to_string(), Ok(b"1234".to_vec()));
    ...
    fn test_read_persistent_value_error() {
        let mut ops = TestOps::default();
        ops.persistent_values
            .insert("foo".to_string(), Err(IoError::Io));
    ...
    fn test_write_persistent_value_error() {
        let mut ops = TestOps::default();
        ops.persistent_values
            .insert("foo".to_string(), Err(IoError::NoSuchValue));
