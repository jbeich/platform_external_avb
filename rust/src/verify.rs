...
use crate::{
    ...
    util::{buffer_to_slice_mut, cstr_to_str, set_pointee},
    IoError,
};
...
pub trait Ops {
    ...
    ///
    /// # Safety
    /// libavb retains a pointer to the given buffer so it must remain valid, unmodified, and
    /// unmoved until the resulting verification data has been dropped.
    fn get_preloaded_partition(&mut self, partition: &str) -> Option<&[u8]> {
        None
    }
}
...
//
// This is a bit tricky in Rust, we can't just cast `Ops` to `void*` and back directly because `Ops`
// is a trait to be implemented by the user, which means we don't know the concrete type at this
// point and must use dynamic dispatch.
//
...
impl<'a> UserData<'a> {
    ...
    //
    // # Safety
    // This UserData object must stay valid and must not move while the returned AvbOps object still
    // exists, as it contains a pointer to the UserData which will be left dangling if the UserData
    // drops or moves.
    //
    ...
    fn create_avb_ops(&mut self) -> AvbOps {
        AvbOps {
...
extern "C" fn read_from_partition(
    ...
    let lifetime = ();
    result_to_io_enum((|| {
        let ops = UserData::from_libavb(&ops);
        match ops.read_from_partition(
            cstr_to_str(partition, &lifetime)?,
            offset,
            buffer_to_slice_mut(buffer, num_bytes, &lifetime)?,
        ) {
            Ok(bytes_read) => {
                set_pointee(out_num_read, bytes_read)?;
                Ok(())
            }
            Err(e) => {
                set_pointee(out_num_read, 0)?;
                Err(e)
            }
        }
    })())
...
extern "C" fn get_preloaded_partition(
    ...
    result_to_io_enum((|| {
        let ops = UserData::from_libavb(&ops);
        match ops.get_preloaded_partition(cstr_to_str(partition, &lifetime)?) {
            Some(contents) => {
                ...
                set_pointee(out_pointer, contents.as_ptr() as *mut u8)?;
                // Truncate here if necessary, we may have preloaded more data than libavb needs.
                set_pointee(out_num_bytes_preloaded, min(contents.len(), num_bytes))?;
            }
            None => {
                set_pointee(out_pointer, ptr::null_mut())?;
                set_pointee(out_num_bytes_preloaded, 0)?;
            }
        }
...
mod tests {
    ...
    fn call_read_from_partition(
        ...
        let mut avb_ops = user_data.create_avb_ops();
        ...
        'a: 'b,
    {
        ...
        *out_buffer =
            buffer_to_slice_mut(out_ptr as *mut _, *out_num_bytes_preloaded, ops).unwrap();
        result
    }
