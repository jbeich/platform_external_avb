...
/// Writes a `Result<(), E>` to a `Formatter` using `Display` formatting.
fn display_result<E: fmt::Display>(
...
/// Example output:
/// * `"part_name": Ok`
/// * `"part_name"(p): Ok` <-- (p) indicates a preloaded partition
/// * `"part_name": Verification failure`
...
/// `AvbSlotVerifyFlags` wrapper.
/// See libavb docs for descriptions of each flag.
///
/// This is just a thin wrapper to provide consistent naming scheme with the rest of the library.
pub struct SlotVerifyFlags;

impl SlotVerifyFlags {
    /// `AVB_SLOT_VERIFY_FLAGS_NONE`
    pub const None: AvbSlotVerifyFlags = AvbSlotVerifyFlags::AVB_SLOT_VERIFY_FLAGS_NONE;
    /// `AVB_SLOT_VERIFY_FLAGS_ALLOW_VERIFICATION_ERROR`
    pub const AllowVerificationError: AvbSlotVerifyFlags =
    ...
    pub const RestartCausedByHashtreeCorruption: AvbSlotVerifyFlags =
        AvbSlotVerifyFlags::AVB_SLOT_VERIFY_FLAGS_RESTART_CAUSED_BY_HASHTREE_CORRUPTION;
    /// `AVB_SLOT_VERIFY_FLAGS_NO_VBMETA_PARTITION`
    pub const NoVbmetaPartition: AvbSlotVerifyFlags =
