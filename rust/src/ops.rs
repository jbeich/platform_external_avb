...
pub trait Ops<'a> {
    ...
    /// The boot nonce is returned. This is part of rot data generated by the libavb.
    fn read_boot_nonce(&mut self) -> IoResult<u64>;
    
    /// The boot loader locked status, user enabled rot status and device eio mode enabled 
    /// statuses are returned. These are used to decide the verified boot state in libavb.
    ...
    /// Return the dice certificate chain. Buffer size must be equal to cert chain size
    /// returned by read_dice_cert_chain_size method.
    fn read_dice_cert_chain(
        &mut self,
        out_buffer: &mut [u8]
    ) -> IoResult<()>;
    
    /// Return the dice certificate chain size.
...
/// Info returned from `read_vb_flow_data`.
#[derive(Clone, Copy, Debug)]
pub struct VbFlowData {
    /// Whether the bootloader is locked.
    pub bootloader_locked: bool,
    /// Whether the user has set the rot.
    pub user_enabled_rot: bool,
    /// Whether the device eio mode is ebaled.
    pub device_eio_mode: bool,
}
    ...
    pub(crate) fn new(ops: &'o mut dyn Ops<'p>) -> Self {
        Self {
            avb_ops: AvbOps {
                ...
                read_boot_nonce: Some(read_boot_nonce),
                read_vb_flow_data: Some(read_vb_flow_data),
                generate_true_random: Some(generate_true_random),
                sign_key_with_cdi_attest: Some(sign_key_with_cdi_attest),
                read_dice_cert_chain: Some(read_dice_cert_chain),
                read_dice_cert_chain_size: Some(read_dice_cert_chain_size),
            },
            cert_ops: AvbCertOps {
...
unsafe extern "C" fn read_boot_nonce(
    ops: *mut AvbOps,
    out_boot_nonce: *mut u64,
) -> AvbIOResult {
    // SAFETY: see corresponding `try_*` function safety documentation.
    result_to_io_enum(unsafe { try_read_boot_nonce(ops, out_boot_nonce) })
}

unsafe fn try_read_boot_nonce(
    ops: *mut AvbOps,
    out_boot_nonce: *mut u64,
) -> IoResult<()> {
    check_nonnull(out_boot_nonce)?;
    ...
    let boot_nonce = avb_ops.read_boot_nonce()?;
        // SAFETY:
    // * we've checked that the pointers are non-NULL.
    // * libavb gives us a properly-allocated `out_*`.
    unsafe {
    	ptr::write(out_boot_nonce, boot_nonce);
   }
   Ok(())
}

unsafe extern "C" fn read_vb_flow_data(
    ops: *mut AvbOps,
    out_boot_loader_locked: *mut bool,
    out_user_enabled_rot: *mut bool,
    out_device_eio_mode: *mut bool,
) -> AvbIOResult {
    // SAFETY: see corresponding `try_*` function safety documentation.
    result_to_io_enum(unsafe { try_read_vb_flow_data(ops, 
    out_boot_loader_locked, out_user_enabled_rot, out_device_eio_mode) })
}

unsafe fn try_read_vb_flow_data(
    ops: *mut AvbOps,
    out_boot_loader_locked: *mut bool,
    out_user_enabled_rot: *mut bool,
    out_device_eio_mode: *mut bool,
) -> IoResult<()> {
    check_nonnull(out_boot_loader_locked)?;
    check_nonnull(out_user_enabled_rot)?;    
    check_nonnull(out_device_eio_mode)?;
    ...
    // * libavb gives us a properly-allocated `out_*`.
    unsafe {
        ptr::write(out_boot_loader_locked, false);
        ptr::write(out_user_enabled_rot, false);
        ptr::write(out_device_eio_mode, false);
    }    
    ...
    // * `ops` is only extracted once and is dropped at the end of the callback.
    let avb_ops = unsafe { as_ops(ops) }?;
    let vb_flow_data = avb_ops.read_vb_flow_data()?;
    
    // SAFETY:
    // * we've checked that the pointers are non-NULL.
    // * libavb gives us a properly-allocated `out_*`.
    unsafe {
    	ptr::write(out_boot_loader_locked, vb_flow_data.bootloader_locked);
        ptr::write(out_user_enabled_rot, vb_flow_data.user_enabled_rot);
        ptr::write(out_device_eio_mode, vb_flow_data.device_eio_mode);
    }
    Ok(())
}
...
unsafe extern "C" fn read_dice_cert_chain_size(
    ops: *mut AvbOps, 
    out_dice_cert_chain_size: *mut usize
) -> AvbIOResult {
    result_to_io_enum(try_read_dice_cert_chain_size(
        ops,
        out_dice_cert_chain_size,
    ))
}

fn try_read_dice_cert_chain_size(
    ops: *mut AvbOps,
    out_dice_cert_chain_size: *mut usize,
)-> IoResult<()> {
    check_nonnull(out_dice_cert_chain_size)?;

    // SAFETY:
    // * we only use `ops` objects created via `OpsBridge` as required.
    // * `ops` is only extracted once and is dropped at the end of the callback.
    let avb_ops = unsafe { as_ops(ops) }?;
    let length = avb_ops.read_dice_cert_chain_size()?;
    
    // SAFETY:
    // * we've checked that the pointers are non-NULL.
    // * libavb gives us a properly-allocated `out_*`.
    unsafe {
        ptr::write(out_dice_cert_chain_size, length);
    }
    Ok(())
}

unsafe extern "C" fn read_dice_cert_chain(
    ops: *mut AvbOps,
    buffer_size: usize,
    out_buffer: *mut u8,
) -> AvbIOResult {
    result_to_io_enum(try_read_dice_cert_chain(
        ops,
        buffer_size,
        out_buffer,
    ))
...
fn try_read_dice_cert_chain(
    ops: *mut AvbOps,
    buffer_size: usize,
    out_buffer: *mut u8,
)-> IoResult<()> {
    check_nonnull(out_buffer)?;
    if buffer_size == 0 {
        return Ok(());
    }
    // SAFETY:
    ...
    // * we only access the contents via the returned slice.
    // * the returned slice is not held past the scope of this callback.
    let output = unsafe { slice::from_raw_parts_mut(out_buffer, buffer_size)};

    // SAFETY:
    // * we only use `ops` objects created via `OpsBridge` as required.
    // * `ops` is only extracted once and is dropped at the end of the callback.
    let avb_ops = unsafe { as_ops(ops) }?;
    avb_ops.read_dice_cert_chain(output)
}
...
fn try_sign_key_with_cdi_attest(
    ...
    // * `ops` is only extracted once and is dropped at the end of the callback.
    let avb_ops = unsafe { as_ops(ops) }?;
    let length = avb_ops.sign_key_with_cdi_attest(pub_key, output)?;
