...
use avb_bindgen::{AvbCertOps, AvbCertPermanentAttributes, AvbHashOps, AvbIOResult, AvbOps};
use core::{
    cmp::min,
...
pub trait Ops<'a> {
    ...
    /// Returns the libavb_sha256_ops ops if supported.
    ///
    /// # Returns
    /// The `HashOps` object, or `None` if not supported.
    fn sha256_ops(&mut self) -> Option<&mut dyn HashOps> {
        None
    }
}
...
pub(crate) struct OpsBridge<'o, 'p> {
    ...
    cert_ops: AvbCertOps,
    /// When using libavb sha256_ops, C `AvbOps`/`AvbHashOps` hold circular pointers to each other.
    sha256_ops: AvbHashOps,
...
impl<'o, 'p> OpsBridge<'o, 'p> {
    pub(crate) fn new(ops: &'o mut dyn Ops<'p>) -> Self {
        Self {
            avb_ops: AvbOps {
                ...
                sha256_ops: ptr::null_mut(), // Set at the time of use.
            ...
            sha256_ops: AvbHashOps {
                ops: ptr::null_mut(), // Set at the time of use.
                init: Some(sha256_init),
                update: Some(sha256_update),
                finalize: Some(sha256_finalize),
            },
            rust_ops: ops,
            _pin: PhantomPinned,
        }
    ...
    pub(crate) fn init_and_get_c_ops<'a>(self: Pin<&'a mut Self>) -> &'a mut AvbOps {
        ...
        if self_mut.rust_ops.sha256_ops().is_some() {
            self_mut.avb_ops.sha256_ops = &mut self_mut.sha256_ops;
            self_mut.sha256_ops.ops = &mut self_mut.avb_ops;
        }
...
/// Similar to `as_ops()`, but for sha256 `HashOps`.
///
/// # Safety
/// Same as `as_ops()`.
unsafe fn as_sha256_ops<'o>(hash_ops: *mut AvbHashOps) -> IoResult<&'o mut dyn HashOps> {
    ...
    // in this library;
    ops.sha256_ops().ok_or(IoError::NotImplemented)
}
...
/// SAFETY:
/// See corresponding `try_*` function docs.
unsafe extern "C" fn sha256_init(hash_ops: *mut AvbHashOps) -> AvbIOResult {
    result_to_io_enum(
        // SAFETY: see corresponding `try_*` function safety documentation.
        unsafe { try_sha256_init(hash_ops) },
    )
}
...
/// SAFETY:
/// TODO
unsafe fn try_sha256_init(hash_ops: *mut AvbHashOps) -> IoResult<()> {
    // SAFETY:
    // TODO
    let sha256 = unsafe { as_sha256_ops(hash_ops) }?;
    sha256.init()
}
...
unsafe extern "C" fn sha256_update(
    ...
    result_to_io_enum(
        // SAFETY: see corresponding `try_*` function safety documentation.
        unsafe { try_sha256_update(hash_ops, data, data_size) },
    )
}
...
/// SAFETY:
/// See corresponding `try_*` function docs.
unsafe extern "C" fn sha256_finalize(
    hash_ops: *mut AvbHashOps,
    out: *mut *const u8,
    size: *mut usize,
) -> AvbIOResult {
    result_to_io_enum(
        // SAFETY: see corresponding `try_*` function safety documentation.
        unsafe { try_sha256_finalize(hash_ops, out, size) },
    )
}
