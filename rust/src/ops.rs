...
impl<'o, 'p> OpsBridge<'o, 'p> {
    ...
    /// Configures the `OpsBridge` and its underlying C structs.
    ///
    /// If the contained `Ops` provides `CertOps`, then the C structs will be configured to support
    /// the cert extensions.
    ///
    /// Pinning is necessary here because we're creating C pointers into `self`, so we cannot allow
    /// `self` to subsequently move or else the pointers would become invalid.
    ///
    /// This must be called before calling `get_c_ops()` for the pointers to be properly set.
    pub(crate) fn initialize<'a>(self: Pin<&'a mut Self>) {
    ...
    /// Returns the underlying `&mut AvbOps`.
    ///
    /// Caution: you must call `initialize()` on the `OpsBridge` first in order to properly set
    /// the C pointers.
    pub(crate) fn get_c_ops<'a>(self: Pin<&'a mut Self>) -> &mut AvbOps {
        // SAFETY: we do not move out of `self_mut`.
        &mut unsafe { self.get_unchecked_mut() }.avb_ops
    }
}
