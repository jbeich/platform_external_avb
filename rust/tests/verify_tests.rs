...
use avb::{
    slot_verify, Descriptor, HashDescriptorFlags, HashtreeErrorMode, IoError, SlotVerifyData,
...
fn one_image_gives_single_descriptor() {
    ...
    let data = result.unwrap();
    assert_eq!(data.vbmeta_data()[0].descriptor_iter().count(), 1);
}
...
fn two_images_gives_two_descriptors() {
    ...
    let data = result.unwrap();
    assert_eq!(data.vbmeta_data()[0].descriptor_iter().count(), 2);
}
...
fn verify_hash_descriptor() {
    ...
    let data = result.unwrap();
    let descriptor = match data.vbmeta_data()[0].descriptor_iter().next() {
        Some(Descriptor::Hash(d)) => d,
        d => panic!("Expected hash descriptor, got {:?}", d),
    };

    assert_eq!(descriptor.partition_name, TEST_PARTITION_NAME);
    assert_eq!(descriptor.salt, decode(TEST_IMAGE_SALT_HEX).unwrap());
    assert_eq!(descriptor.digest, decode(TEST_IMAGE_DIGEST_HEX).unwrap());
    assert_eq!(descriptor.image_size() as usize, TEST_IMAGE_SIZE);
    assert_eq!(descriptor.hash_algorithm().unwrap(), TEST_IMAGE_HASH_ALGO);
    assert_eq!(descriptor.flags(), HashDescriptorFlags(0));
}
