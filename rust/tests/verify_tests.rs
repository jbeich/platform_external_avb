...
fn one_image_gives_single_descriptor() {
    ...
    let data = result.unwrap();
    assert_eq!(data.vbmeta_data()[0].get_descriptors().unwrap().len(), 1);
}
...
fn two_images_gives_two_descriptors() {
    ...
    let data = result.unwrap();
    assert_eq!(data.vbmeta_data()[0].get_descriptors().unwrap().len(), 2);
}
...
fn verify_hash_descriptor() {
    ...
    let data = result.unwrap();
    let descriptors = &data.vbmeta_data()[0].get_descriptors().unwrap();
