...
fn rollback_violation_fails_verification() {
    ...
    let error = result.unwrap_err();
    assert!(matches!(error, SlotVerifyError::RollbackIndex(_)));
}
...
fn untrusted_vbmeta_keys_fails_verification() {
    ...
    let error = result.unwrap_err();
    assert!(matches!(error, SlotVerifyError::PublicKeyRejected(_)));
}
...
fn invalid_public_key_verification_data_provided() {
    ...
    let error = result.unwrap_err();
    let data = match error {
        SlotVerifyError::PublicKeyRejected(Some(data)) => data,
        _ => panic!("Expected verification data to exist"),
    };
    assert_eq!(
        format!("{data}"),
...
fn invalid_rollback_index_verification_data_provided() {
    ...
    let error = result.unwrap_err();
    let data = match error {
        SlotVerifyError::RollbackIndex(Some(data)) => data,
        _ => panic!("Expected verification data to exist"),
    };
    assert_eq!(
        format!("{data}"),
