...
fn verify_chain_partition_descriptor() {
    ...
    // * a raw test image in TEST_PARTITION_NAME
    // * a signed test image with vbmeta footer in TEST_PARTITION_2_NAME
    // * a vbmeta image in "vbmeta" which:
    //   * signs the TEST_PARTITION_NAME image
    //   * chains to TEST_PARTITION_2_NAME
    let mut ops = test_ops_two_images_one_vbmeta();
    // Replace vbmeta with the version containing a chain partition descriptor.
    ops.add_partition(
        "vbmeta",
    ...
    ops.rollbacks
        .insert(TEST_CHAINED_PARTITION_ROLLBACK_LOCATION, 0);
    ...
    // Grab the chain partition descriptor and make sure it looks like we expect.
    let descriptors = vbmeta.descriptors().unwrap();
    let descriptor = descriptors
        .iter()
        .filter_map(|d| match d {
            Descriptor::ChainPartition(c) => Some(c),
            _ => None,
        })
        .next()
        .unwrap();
