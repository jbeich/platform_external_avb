...
AvbSlotVerifyResult avb_rot_generate_rot_data(AvbOps* ops,
  ...
  const char* certificateSubject = "AVB_ROT_CERT_SUBJECT";
  ...
  // Generate DICE attested certificate. The dice certificate does not
  // include the cbor tag 0xd2 at the top. So just add that
  rotSigningKeyCert[0] = 0xD2;
  ...
  actualCertLen++;  // account for the cbor tag at the beginning.
