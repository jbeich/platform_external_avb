...
static bool avb_rot_dice_derive_cert_id(const uint8_t* cdi_public_key,
...
bool avb_rot_dice_generate_certificate(
  ...
  *certificate_actual_size = 0;
  ...
  if (!avb_rot_dice_derive_cert_id(
          authority_public_key, ED25519_PUBLIC_KEY_LEN, authority_id)) {
    return false;
  }
...
bool avb_rot_generate_rot_data(
    AvbOps* ops,
    const AvbSlotVerifyData* slotData,
    uint8_t signedRotData[AVB_ROT_MAX_DATA_SIZE],
    size_t* actualRotDataLen,
    uint8_t signingKeyCert[ROT_SIGNING_KEY_CERT_SIZE],
    size_t* actualCertLen) {
  const char* certificateSubject = "AVB_ROT";
  avb_assert(ops->generate_true_random != NULL);
  avb_assert(ops->read_rot_data != NULL);
  avb_assert(ops->generate_true_random != NULL);
  if (ops->generate_true_random == NULL ||
      ops->sign_key_with_cdi_attest == NULL || ops->read_rot_data == NULL) {
    return false;
  }
  bool ret = true;
  uint64_t bootNonce = 0;
  vb_state_t state = unverified;
  ...
  if (rotPublicKey == NULL || rotPrivateKey == NULL || seed == NULL ||
      ...
      signedRotData == NULL || signature == NULL) {
    ret = false;
    goto generateRotDataErr;
  }

  // Read boot parameters
  if (ops->read_rot_data(ops,
                         &bootNonce,
                         &state,
                         &bootLoaderLocked,
                         &osVersion,
                         &osPatchLvl,
                         &bootPatchLvl,
                         &vendorPatchLvl) != AVB_IO_RESULT_OK) {
    ret = false;
    goto generateRotDataErr;
  }
  ...
  // Generate keypairs
  if (ops->generate_true_random(ops, seed) != AVB_IO_RESULT_OK) {
    ret = false;
    goto generateRotDataErr;
  }
  ...
  *actualRotDataLen = avb_rot_cbor_encode_signed_rot_data(
      rotData, rotDataLen, signature, signedRotData);
  if (*actualRotDataLen < 0) {
    ret = false;
    goto generateRotDataErr;
  }
