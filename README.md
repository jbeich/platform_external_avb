...
* `AVB_HASHTREE_ERROR_MODE_MANAGED_RESTART_AND_EIO` means that either `restart`
...
`androidboot.vbmeta.invalidate_on_error`, and
`androidboot.vbmeta.veritymode.managed` kernel command-line parameters in the
following way:

|      | `androidboot.veritymode` | `androidboot.vbmeta.veritymode.managed` | `androidboot.vbmeta.invalidate_on_error` |
|------|:------------------------:|:---------------------------------------:|:----------------------------------------:|
| `AVB_HASHTREE_ERROR_MODE_RESTART_AND_INVALIDATE` | **restart** | (not set) | **yes** |
| `AVB_HASHTREE_ERROR_MODE_RESTART` | **restart** | (not set) | (not set) |
| `AVB_HASHTREE_ERROR_MODE_EIO` | **eio** | (not set) | (not set) |
| `AVB_HASHTREE_ERROR_MODE_MANAGED_RESTART_AND_EIO` | **eio** or **restart** | **yes** | (not set) |
| `AVB_HASHTREE_ERROR_MODE_LOGGING` | **ignore_corruption** | (not set) | (not set) |

The only exception to table is that if that the
`AVB_VBMETA_IMAGE_FLAGS_HASHTREE_DISABLED` is set in the top-level vbmeta, then
`androidboot.veritymode` is set to **disabled** and
`androidboot.vbmeta.veritymode.managed` and
`androidboot.vbmeta.invalidate_on_error` will be unset.
