...
chained `vbmeta` partition itself - can be updated as a group,
independently of the rest of the partitions. Although this can be similarly
done by setting up all such partitions as chained partitions, chaining
`vbmeta` partition provides additional flexibility. First, partitions in a
updatable group are logically bundled together, which shows a better
relationship between partitions. Second, partitions managed by chained
`vbmeta` partitions no longer need to be set up as chained partitions. This
allows `libavb` to better incorporate with devices using logical
partitions. Because the bootloader of such a device may not have the
capability of accessing HLOS-managed logical partitions. Having fixed-size
`vbmeta` partitions - which are not set up as logical partitions, as the
sizes for them are typically small - allows the bootloader to be able to
read the VBMeta structs for all the partitions.
...
not `vbmeta.img`. With the above setup, paritions `system.img`,
