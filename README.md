...
rollback indexes are `[42, 101]` which for slot B they are `[43,
103]`.
...
if the HLOS has tampered with the data, e.g. if it has overwritten it.
...
booting. In pseudo-code it would look like this:
...
* If the device is UNLOCKED, there is no requirement to check the key
  ...
  about verification not occurring.
    + It is device-dependent how this is implemented since it depends
      ...
      is to be shown until the user dismisses it. Care must be taken
      to avoid screen burn-in issues - one reasonable option is to
      just power off the device after, e.g. 30 seconds. If the device
      does not have a screen, other ways must be used to convey that
      the device is UNLOCKED (lightbars, LEDs, etc.).
