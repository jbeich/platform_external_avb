...
Specifically, additional `vbmeta` partitions can be created and chained
into top-level `vbmeta.img`, in order to manage partitions in updatable
groups. A chained `vbmeta` partition includes the hash or hash-tree
descriptors for the partitions in the group, all of which - along with the
chained `vbmeta` partition itself - can be updated independently of the
rest of the partitions. Although this can be similarly done by setting up
all such partitions as chained partitions, chaining `vbmeta` partition
provides additional flexibility. First, partitions in a updatable group are
logically bundled together, which shows a better relationship between
partitions. Second, partitions managed by chained `vbmeta` partitions no
longer need to be set up as chained partitions. This allows `libavb` to
better incorporate with devices using logical partitions. Because the
bootloader of such a device may not have the capability of accessing the
HLOS-managed logical partitions. Having fixed-size `vbmeta` partitions -
these partitions are not set up as logical partitions, as the sizes for
them are typically small - allows the bootloader to be able to read the
VBMeta structs for all the partitions.
...
`product_services.img` will be included in `vbmeta_mainline.img` only. With
the example configuration, paritions `system.img`, `product_services.img`
and `vbmeta_mainline.img` can be updated independently - but as a group -
of the rest of the partitions, *or* as part of the traditional updates that
