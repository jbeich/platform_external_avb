...
* `AVB_HASHTREE_ERROR_MODE_MANAGED_RESTART_AND_EIO` means that either
   the `restart` or `eio` is used depending on state. This mode implements a
   state machine whereby `restart` is used by default and when
   the `AVB_SLOT_VERIFY_FLAGS_RESTART_CAUSED_BY_HASHTREE_CORRUPTION` is
   passed to `avb_slot_verify()` the mode transitions to `eio`. When a new
   OS has been detected the device transitions back to the `restart` mode.
...
| mode | `androidboot.veritymode` | `androidboot.vbmeta.veritymode.managed` | `androidboot.vbmeta.invalidate_on_error` |
|:----:|:----------------------:|:-------------------------------------:|:--------------------------------------:|
