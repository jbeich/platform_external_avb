...
storage of what state the bootloader is currently in. This would need
to include transition rules e.g. if the kernel indicates that it
rebooted because of a `dm-verity` error the bootloader would need to
transition from the `restart` state to the `eio` state. Ditto, when
the slot is updated the bootloader needs to transition from the `eio`
state to the `restart` state so the user doesn't have to click through
a screen on every boot.
