...
struct and verify that it was signed by `key1_pub` (stored in the
...
fixed. Additionally the device stores the last seen rollback index:
...
having the device intelligently manage `stored_rollback_index[n]`.
...
In addition to the system primities, `libavb` interfaces with the boot
...
The device must only allow transitioning from LOCKED to UNLOCKED after
asserting physical presence of the user. If the device has a display
and buttons this is typically done by showing a dialog and requiring
the user to confirm or cancel using physical buttons.
...
`userdata` and `stored_rollback_index[n]` locations) shall also
happening when transitioning from UNLOCKED to LOCKED. If the device is
required to use full disk encryption, then a less intensive wipe is
required for UNLOCKED to LOCKED. Depending on the device form-factor
and intended use, the user should be prompted to confirm before any
data is erased.

## Tamper-evident Storage

In this document, tamper-evident means that it's possible to detect if
the HLOS has tampered with the data, e.g. if it has overwritten it.

Tamper-evident storage must be used for stored rollback indexes, keys
used for verification, and device state. If tampering has been
detected the corresponding `AvbOps` operation should fail by
e.g. returning `AVB_IO_RESULT_ERROR_IO`. It is especially important
that verification keys cannot be tampered with since they represent
the root-of-trust.

If verification keys are mutable (for example, if a verification key
can be set via `fastboot`) they must only be set by the end user,
e.g. it must never be set at the factory or store or any intermediate
point before the end user. Additionally, it must only be possible to
set a key while the device is in the UNLOCKED state.
...
booting a slot.  If not using A/B this is straightforward - just
update it to what's in the AVB metadata for the slot before
booting. Here's the pseudo-code for that:
    ...
    for (int n = 0; n < AVB_MAX_NUMBER_OF_ROLLBACK_INDEX_LOCATIONS; n++) {
		...
		if (current_stored_rollback_index < rollback_index) {
...
implementation. Note however that this requires verifying *all*
bootable slots at every boot and this may impact boot time.
...
* If the device is LOCKED, only an OS signed by an embedded key (see
  the previous section) shall be accepted. Additionally, the rollback
  indexes MUST not exceed those stored on the device and
  `stored_rollback_index[n]` on the device are expected to be updated
  as specified in the previous section.

* If the key used for verification was set by the end user, (e.g. it
  can be set via `fastboot`) and the device has a screen, it must show
  a warning with the key fingerprint to convey that the device is
  booting a custom OS. The warning must be shown for at least 10
  seconds before the boot process continues. If the device does not
  have a screen, other ways must be used to convey that the device is
  booting a custom OS (lightbars, LEDs, etc.).

* If the device is UNLOCKED, there is no requirement to check the key
  used to sign the OS nor is there any requirement to check rollback
  indexes (or update stored_rollback_index[n] on the device). Because
  of this the user must always be shown a warning about verification
  not occurring.
    + It is device-dependent how this is implemented since it depends
      ...
      a screen and buttons (e.g. if it's a phone) the warning is to be
      shown until the user dismisses it. Care must be taken to avoid
      screen burn-in issues - one reasonable option is to just power
      off the device after, e.g. 30 seconds. If the device does not
      have a screen, other ways must be used to convey that the device
      is UNLOCKED (lightbars, LEDs, etc.).
