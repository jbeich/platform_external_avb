...
By design, hashtree verification errors are detected by the HLOS not
the bootloader. AVB provides a way to specify how the error should be
handled through the `hashtree_error_mode` parameter in the
...
This depends entirely on the device, the quality of the underlying
storage, how the device is intended to be used, and the desired user
experience.
...
the "Recovering from dm-verity errors" section) in a boot loader, a
combination of `AVB_HASHTREE_ERROR_MODE_RESTART` and
`AVB_HASHTREE_ERROR_MODE_EIO` could be used along with persistent
storage of whether the bootloader is currently in. This would need to
include transition rules e.g. if the kernel indicates that it rebooted
because of a dm-verity error the bootloader would need to transition
from `restart` to `eio`. Ditto, when the slot is updated the
bootloader needs to transition from `eio` to `restart` so the user
doesn't have to click through a screen on every boot.

On the other hand, if the device doesn't have a screen or if the HLOS
supports multiple bootable slots it may make more sense to just use
